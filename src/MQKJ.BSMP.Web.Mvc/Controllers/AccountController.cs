...
using MQKJ.BSMP.Web.Models.Account;
using MQKJ.BSMP.Web.Views.Shared.Components.TenantChange;
using MQKJ.BSMP.Roles;
...
{
    public class AccountController : BSMPControllerBase
    {
        ...
        private readonly IUserNavigationManager _userNavigation;
        ...
        public AccountController(
            ...
            IUserNavigationManager userNavigation)
        {
        ...
        public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            var menus = await _userNavigation.GetMenusAsync(loginResult.User.ToUserIdentifier());
        ...
        #region 修改密码
        [AbpAuthorize]
        public async Task<ActionResult> ModifyPassword(string oldPassword,string newPassword)
        {
            ...
            try
            {
                ...
                {
                    throw new UserFriendlyException("新旧密码不能相等");
                }

                var userId = AbpSession.UserId;
                var user = await _userManager.GetUserByIdAsync(userId.Value);

                var isRight = await _userManager.CheckPasswordAsync(user, oldPassword);
                if (!isRight)
                {
